...
function FormularioProducto({ onAgregar }) {
    ...
    const validarFormulario = () => {
        ...
        if (!producto.nombre.trim()) {
            nuevosErrores.nombre = 'El nombre del artista es obligatorio.';
        }
        if (!producto.disco.trim()) {
            nuevosErrores.disco = 'El nombre del disco es obligatorio.';
        }
        if (!producto.formato.trim()) {
            nuevosErrores.formato = 'El formato del disco es obligatorio.';
        }
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <div>
                <label>Disco:</label>
                <input
                    type="text" name="disco" value={producto.disco} onChange={handleChange} required />
                {errores.disco && <p style={{ color: 'red' }}>{errores.disco}</p>}
            </div>
            <div>
                <label>Precio:</label>
            ...
            <div>
                <label>Formato:</label>
                <input
                    type="text" name="formato" value={producto.formato} onChange={handleChange} required />
                {errores.formato && <p style={{ color: 'red' }}>{errores.formato}</p>}
            </div>
